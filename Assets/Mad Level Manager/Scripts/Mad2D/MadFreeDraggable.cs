        ...
        if (!IsTouchingSingle()) {
            ...
            if (scaleMode == ScaleMode.Free) {
                ...
                if (scaleModifier != 0) {
                    ...
                    float mod = scaleTarget.x / scaleSource.x;
                    var local = cachedCamPos;
                    var newLocal = local * mod;
                    MoveToLocal(newLocal, scaleEasingType, scaleEasingDuration);
                }
            }
            ...
            if (IsTouchingJustStarted()) {
                ...
                cachedCamPos -= ApplyScreenAspect(touchPos - lastPosition);
                lastPosition = touchPos;
            }

            RegisterDelta(ApplyScreenAspect(cachedCamPos - lastCamPos));
